...
vi config/targets.conf   # PRIMARY_TARGET (victim, not the gateway)

# 3. Set up passwordless SSH from Kali to the gateway
...
# 6. Override gateway/target on the fly
...
Defines victim hosts — the machines **behind** the gateway that receive attack
...
All collectors run on Kali and SSH/SCP into the gateway:
...
├── config/
│   ├── lab.conf            Gateway SSH params + Kali interface

...
check_deps() {
    ...
    local packages=(nmap hping3 curl wget ncat tshark tcpdump openssh-client jq python3)
    local commands=(nmap hping3 curl ncat tshark tcpdump ssh jq python3)
    local missing_cmds=()
    local missing_pkgs=()

    for i in "${!commands[@]}"; do
        local cmd="${commands[i]}" pkg="${packages[i]}"
    ...
    if [[ ${#missing_pkgs[@]} -gt 0 ]]; then
        ...
        if command -v apt-get &>/dev/null; then
            warn "Attempting install: sudo apt-get update && sudo apt-get install -y ${missing_pkgs[*]}"
            if ! sudo apt-get update -qq || ! sudo apt-get install -y "${missing_pkgs[@]}"; then
                fail "Failed to install required packages: ${missing_pkgs[*]}"
                exit 1
            fi
            section "Dependency Check"
            log "Installed missing packages: ${missing_pkgs[*]}"
        else
            fail "apt-get is not available; cannot install required packages: ${missing_pkgs[*]}"
            exit 1
        fi
    fi
}
